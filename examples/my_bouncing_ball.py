...
class BouncingBallExample(nn.Module):
    ...
    def forward(self, t, state):
        ...
        dpos = vel
        dvel = -self.gravity
        return torch.stack((dpos, dvel.expand(dpos.size()), torch.zeros_like(log_radius)))
    ...
    def get_initial_state(self, init_pos):
        state = torch.stack((init_pos, self.init_vel.expand(init_pos.size()), self.log_radius.expand(init_pos.size())), dim=0)
    ...
    def state_update(self, state):
        ...
        return torch.stack((pos, vel, log_radius))

    def get_collision_times(self, init_pos, nbounces=1):

        event_times = []

        t0, state = self.get_initial_state(init_pos)
        ...
        for i in range(nbounces):
            ...
            state = self.state_update(solution[-1])
            t0 = event_t

        return event_times

    def simulate(self, init_pos, nbounces=1):
        event_times = self.get_collision_times(init_pos, nbounces)
        ...
        for event_t in event_times:
            ...
            state = self.state_update(solution[-1])
            t0 = event_t
        return torch.cat(times), torch.cat(trajectory, dim=0)[:, 1].reshape(-1), torch.cat(velocity, dim=0)[:, 1].reshape(-1), event_times


def gradcheck(init_pos, nbounces):
    ...
    event_t = system.get_collision_times(init_pos, nbounces)[-1]
    event_t.backward()
    ...
    analytical_grads['init_pos'] = init_pos.grad
    ...
    for var, param in variables.items():
        orig = param.data
        param.data = orig - eps
        f_meps = system.get_collision_times(init_pos, nbounces)[-1]
        param.data = orig + eps
        f_peps = system.get_collision_times(init_pos, nbounces)[-1]
        param.data = orig
...
if __name__ == "__main__":
    ...
    init_pos = torch.tensor((10., 9.), requires_grad=True)

    # gradcheck(init_pos, args.nbounces)

    system = BouncingBallExample()
    times, trajectory, velocity, event_times = system.simulate(init_pos=init_pos, nbounces=args.nbounces)
